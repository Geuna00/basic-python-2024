...
class BackWorker(QThread): # PyQt에서 스레드 클래스 상속
    ...
    def run(self) -> None: # 스레드 실행
        # 스레드로 동작할 내용
        maxVal = 100001
        self.initSignal.emit(maxVal) # emit() : 값을 넘겨줌
        ## self.parent.pgbTask.setValue(0) # 프로그레스바 0부터 시작 / !!! QThread에서는 UI관련된 처리를 할 수 없음 !!!
        ## self.parent.pgbTask.setRange(0, maxVal-1) # 0부터 100까지
        for i in range(maxVal):
            print_str = f'스레드 출력 >> {i}'
            print(print_str)
...
class qtwin_exam(QWidget): # UI 스레드
    ...
    def btnStartClicked(self):
        ...
        th.setLog.connect(self.setTxbLog) # TextBrowser 위젯에 진행상황 출력
    ...
    @pyqtSlot(str) # BackWorkder 스레드에서 self.setLog.emit() 이 동작해서 실행
    def setTxbLog(self, msg):
        self.txbLog.append(msg)

    @pyqtSlot(int) # BackWorkder 스레드에서 self.setSignal.emit() 이 동작해서 실행
